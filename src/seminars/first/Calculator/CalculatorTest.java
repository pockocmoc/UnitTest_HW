...
public class CalculatorTest {
	public static void main(String[] args) {
		...
		// Проверяем метод calculatingDiscount
		double result1 = Calculator.calculatingDiscount(100, 20);
		Assertions.assertThat(result1).isEqualTo(80);

		// Проверяем метод calculatingDiscount с нулевой суммой покупки
		double result2 = Calculator.calculatingDiscount(0, 20);
		Assertions.assertThat(result2).isEqualTo(0);

		// Проверяем метод calculatingDiscount с нулевой суммой скидки
		double result3 = Calculator.calculatingDiscount(100, 0);
		Assertions.assertThat(result3).isEqualTo(100);

//		// Проверяем метод calculatingDiscount с отрицательной суммой покупки
//		Assertions.assertThatThrownBy(() -> Calculator.calculatingDiscount(-100, 20))
//				.isInstanceOf(ArithmeticException.class)
//				.hasMessage("Сумма покупки, не может быть отрицательной");
		
//		// Проверяем метод calculatingDiscount с отрицательной суммой скидки
//        Assertions.assertThatThrownBy(() -> Calculator.calculatingDiscount(100, -20))
//                .isInstanceOf(ArithmeticException.class)
